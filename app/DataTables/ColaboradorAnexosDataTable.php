...
class ColaboradorAnexosDataTable extends DataTable
{
    public function dataTable($query)
    {
        return datatables($query)
            ->addColumn('action', function (Anexo $anexo) {
                $rota = route('colaborador.excluir.anexo', $anexo->id);
                $rota_edit = route('colaborador.edit', $anexo->id);
                $rota_show = route('colaborador.baixar.anexo', $anexo->id);
                $token = csrf_token();
                return "
                <style>
                    #link-grupo{
    ...
    protected function getColumns()
    {
        ...
        return [
            ...
            'dono_anexo' => ['title' => 'COLABORADOR/DEPENDENTE(S)'],
    ...
    {
        return ' ColaboradoresAnexos_ ' . date(' YmdHis');
    }
}